<template>
  <v-container elevation-0 row justify-center>
    <div my-4 class="white component-wrapper">
      ...
      <h1 class="rozliczenia__header">Rozliczenia > Specjalistyka > {{ pacjent.pacjent.imie}} {{pacjent.pacjent.nazwisko}}</h1>
      ...
      <div class="rozlcizenia">
        <ul>
          ...
          <li
            v-for="(wizyta) in pacjent.wizyty"
            :key="wizyta"
...
export default {
  ...
  mounted: function() {
   const pacjentId = this.$route.params.pacjent
    apiService.getRozliczeniaByPacjentId(pacjentId).then(response => {
      this.$store.commit('GET_ROZLICZENIA_PACJENT', response.data)
      this.pacjent = this.$store.getters.getRozliczeniaPacjent
    })
  },
  updated: function() {
